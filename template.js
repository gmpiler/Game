...
window.onload = function () {
	...
	//ぞう山くん画像
	const kirimochiImgUrl = "kirimochi.png";						//game.htmlからの相対パス
	game.preload([kirimochiImgUrl]);					//データを読み込んでおく
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		//ポイント表示テキスト2
		const sampleText = new Label();
		sampleText.font = "20px Meiryo"
		sampleText.color = 'rgba(100, 100, 100, 5)'
		sampleText.width = 200;
		sampleText.moveTo(point, point);
		mainScene.addChild(sampleText);

		sampleText.text = "sampleText" + point;

		//ぞう山ボタン
		const kirimochiImg = new Sprite(150, 138);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		kirimochiImg.moveTo(118, 100);						//ぞう山ボタンの位置
		kirimochiImg.image = game.assets[kirimochiImgUrl];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		mainScene.addChild(kirimochiImg);					//mainSceneにこのぞう山画像を貼り付ける

		kirimochiImg.ontouchend = function () {				//ぞう山ボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			point++;									//Pointを1増やす
			game.assets[clickSndUrl].clone().play();		//クリックの音を鳴らす。

			//クリックしたのでぞう山画像のｘ位置を戻す
			this.x = -200;							//this.xって何？と思った方、kirimochiの関数内でぞう山の座標を動かすときにはthisを使います。
		...
		game.onenterframe = function () {
			if (state == 0) { 							//state=0のとき、初期セット状態(Pointの状態を０にして)
				kirimochiImg.x = -200;						//ぞう山のｘ座標を指定
				kirimochiImg.y = 100;						//ぞう山のy座標を指定
				point = 0;  							//point初期化
			...
			if (state == 3) {							//状態３（point６以上から）
				kirimochiImg.x += 10;
				kirimochiImg.y = 200 + Math.sin(kirimochiImg.x / 70) * 100; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			if (state == 4) {							//状態４（point９以上から）　4は初期セット状態（state=4）と移動状態（state=4.1)の2つに状態をわける		
				kirimochiImg.y = Math.random() * 400;			//ｙ座標の位置をランダムに決定
				state = 4.1;
			}
			if (state == 4.1) {							//状態４．１ 移動状態
				kirimochiImg.x += 10;						//ただ移動します
			}
			if (state == 5) {							//状態５（point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				kirimochiImg.x += 20;						//移動します。
				kirimochiImg.y = Math.random() * 400;			//ｙ座標の位置を枚フレーム毎にランダム決定
			}
			...
			//ゲームオーバー判定
			if (kirimochiImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
